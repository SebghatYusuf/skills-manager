...
export function createOpencodeTarget(options: { workspaceFolders: string[] }): SkillTarget {
  const homeRoot = path.join(os.homedir(), ".config", "opencode", "skill");
  const workspaceRoots = options.workspaceFolders.map((folder) => path.join(folder, ".opencode", "skill"));
  ...
  return {
    ...
    async setEnablement(skill: SkillRecord, enabled: boolean): Promise<void> {
      ...
      if (!root) {
        throw new Error("Skill is not installed in a known OpenCode root.");
      }
      const disabledRoot = getDisabledRoot(root);
