...
export function createCursorTarget(): SkillTarget {
  const homeRoot = path.join(os.homedir(), ".cursor", "skills");
  ...
  return {
    ...
    async setEnablement(skill: SkillRecord, enabled: boolean): Promise<void> {
      ...
      if (!root) {
        throw new Error("Skill is not installed in the Cursor root.");
      }
      const disabledRoot = getDisabledRoot(root);
